 ...
 * Decemeber 18th, 2018
 */
#ifndef TRADING_SYSTEM_SOA_HPP
#define TRADING_SYSTEM_SOA_HPP
...
/**
 * Definition of a generic base class ServiceListener to listen to add, update, 
 * and remove events on a Service. This listener should be registered on a 
 * Service for the Service to notify all listeners for these events.
 */
template<typename V>
...
public:
    ...
    // Add a listener to the Service for callbacks on add, remove, 
    // and update events for data to the Service.
 ...
 * Services can invoke the Publish() method on this Service to publish data to 
 * the Connector for a publisher Connector.
 * Note that a Connector can be publisher-only, subscriber-only, 
